...
# TODO: Implement file_semantic_model.py for advanced file analysis
...
async def _get_db_adapter():
    ...
    if _postgres_adapter is None:
        _postgres_adapter = PostgreSQLAdapterAsync(
            database_url=config.database_url,
            schema=os.getenv('DEMENTIA_SCHEMA')  # Will auto-detect if not set
        )
        await _postgres_adapter.ensure_schema_exists()
...
async def _get_cached_adapter(schema_name: str) -> PostgreSQLAdapterAsync:
    """
...
async def _init_local_session():
    """
    Initialize local MCP session with PostgreSQL persistence (ASYNC).
    ...
    adapter = await _get_db_adapter()
    _session_store = PostgreSQLSessionStoreAsync(adapter)

    # Create session in database
    try:
        result = await _session_store.create_session(
        ...
        # CRITICAL: Close pool and reset adapter so it can be recreated in the server's event loop
        # asyncio.run() creates a temporary loop that closes. The pool is bound to it.
        # We must close this pool and let the server create a new one in its own loop.
        global _postgres_adapter, _adapter_cache
        if _postgres_adapter:
            pool = await _postgres_adapter.get_pool()
            await pool.close()
            _postgres_adapter = None
            _adapter_cache.clear()
            print("🔄 Database pool closed (ready for server loop)", file=sys.stderr)
...
async def _auto_load_handover() -> Optional[str]:
    """
    Automatically load the last handover on first tool call (ASYNC).
    ...
    try:
        ...
        async with pool.acquire() as conn:
            await conn.execute(f"SET search_path TO {adapter.schema}")
            handover = await conn.fetchrow("""
        ...
        if handover:
            handover_data = json.loads(handover['metadata'])  # metadata is first column
            hours_ago = (time.time() - handover['timestamp']) / 3600  # timestamp is second column
...
async def _check_project_selection_required(project: Optional[str] = None) -> Optional[str]:
    """
    Check if project selection is required before tool execution (ASYNC).
    ...
    handover_msg = await _auto_load_handover()
    if handover_msg:
    ...
    if project:
        ...
        try:
            projects = await _session_store.get_projects_with_stats()
...
def _get_project_for_context(project: str = None) -> str:
    ...
    global _postgres_adapter
    if _postgres_adapter and _postgres_adapter.schema and _postgres_adapter.schema != 'default':
...
class AsyncAutoClosingConnection:
    ...
    async def commit(self):
        """No-op for asyncpg (auto-commit), kept for compatibility during migration."""
        pass
...
async def _get_db_for_project(project: str = None):
    """
    Get database connection for a specific project (ASYNC).

    Args:
        project: Project name (uses fallback logic if not provided)

    Returns:
        AsyncAutoClosingConnection for the specified project
    """
    target_project = _get_project_for_context(project)

    # If it's the same as our global adapter, reuse it
    global_adapter = await _get_db_adapter()
    if target_project == global_adapter.schema:
        return await get_db()

    # Get cached adapter for different project (prevents connection pool exhaustion)
    adapter = await _get_cached_adapter(target_project)
    pool = await adapter.get_pool()
...
async def get_db():
    """
    Get PostgreSQL database connection with auto-cleanup (ASYNC).
    ...
    Returns AsyncAutoClosingConnection with:
    - Schema isolation via search_path (global adapter's schema)
    - Dict-like rows via asyncpg Record
...
async def _get_session_id_for_project(conn, project_name: Optional[str] = None) -> str:
    """
    Get the most recent session ID for a given project from the correct schema (ASYNC).

    Args:
        conn: Async database connection (already connected to correct schema)
...
async def _fetch_project_stats_async(conn, schema: str) -> Optional[dict]:
    """
    Check if project schema exists and fetch statistics (ASYNC).
    ...
    Returns:
        dict with {"sessions": int, "contexts": int, "memories": int} if schema exists
    ...
    stats = {"sessions": 0, "contexts": 0, "memories": 0}

    # Check for tables and count if they exist
    for table_name in ["sessions", "context_locks", "memory_entries"]:
        table_exists_row = await conn.fetchrow(f"""
            SELECT EXISTS (
                SELECT 1
                FROM information_schema.tables
        ...
        if table_exists_row and table_exists_row['exists']:
            count_row = await conn.fetchrow(f'SELECT COUNT(*) as count FROM "{schema}".{table_name}')
            stats[table_name.replace('_entries', '')] = count_row['count']
...
async def switch_project(name: str) -> str:
    ...
    try:
        ...
        try:
            updated = await _session_store.update_session_project(session_id, safe_name)
            if updated:
        ...
        global_adapter = await _get_db_adapter()
        pool = await global_adapter.get_pool()
        async with pool.acquire() as conn:
            stats = await _fetch_project_stats_async(conn, safe_name)
...
async def create_project(name: str) -> str:
    ...
    try:
        ...
        adapter = await _get_cached_adapter(safe_name)

        # Check if schema already exists
        # We need a connection to the default database to check information_schema.
        global_adapter = await _get_db_adapter()
        pool = await global_adapter.get_pool()
        ...
        return json.dumps({
            ...
            "project": name,
            "schema": safe_name,
            "usage": f"Use select_project_for_session('{name}') to switch to this project"
        })
...
async def list_projects() -> str:
    """
    List all available projects and their statistics.

    Returns:
        JSON list of projects with stats
    """
    global _session_store

    if not _session_store:
        return "Error: Session management not initialized."

    try:
        projects = await _session_store.get_projects_with_stats()
        
        # Filter out __PENDING__
        visible_projects = [p for p in projects if p['project_name'] != '__PENDING__']
        
        if not visible_projects:
            return "No projects found. Create one with create_project('name')."
...
async def get_project_info(name: str) -> str:
    ...
    try:
        ...
        # Get cached adapter for specific schema
        adapter = await _get_cached_adapter(safe_name)
        pool = await adapter.get_pool()

        async with pool.acquire() as conn:
            # Check if schema exists (by trying to set search path or query tables)
            # We can check information_schema from global adapter, but here we are already connected to schema?
            # No, pool connects to database, we set search_path.
            
            # Actually, let's use the global adapter to check schema existence first if we want to be sure.
            # But _get_cached_adapter already ensures schema exists.
            # So we can proceed.

            # Get project stats
            # We need to set search path manually if not using AsyncAutoClosingConnection wrapper
            # But we can use the wrapper!
            pass

        # Let's use AsyncAutoClosingConnection for cleaner code
        async with AsyncAutoClosingConnection(pool, safe_name) as conn:
             # Get project stats
            rows = await conn.execute(f'SELECT COUNT(*) as count FROM "{safe_name}".sessions')
            total_sessions = rows[0]['count']

            rows = await conn.execute(f'SELECT COUNT(*) as count FROM "{safe_name}".context_locks')
            total_contexts = rows[0]['count']

            rows = await conn.execute(f'SELECT COUNT(*) as count FROM "{safe_name}".memory_entries')
            total_memories = rows[0]['count']
    ...
    try:
        ...
        if not confirm:
            return json.dumps({
                ...
                "instruction": f"Call delete_project(name='{name}', confirm=True) to proceed"
            })

        # Connect and delete schema using global adapter
        global_adapter = await _get_db_adapter()
        pool = await global_adapter.get_pool()
        ...
        async with pool.acquire() as conn:
            ...
            await conn.execute(f'DROP SCHEMA "{safe_name}" CASCADE')
...
@mcp.tool()
async def lock_context(topic: str, content: str, priority: str = "normal", tags: str = None, project: str = None) -> str:
    """
    Lock a new context or update an existing one.

    Args:
        topic: Unique label for the context (e.g., "auth_flow", "database_schema")
        content: The full content to lock
        priority: Priority level (high, medium, low, normal)
        tags: Comma-separated tags (e.g., "auth, security, api")
        project: Project name (optional)
    """
    if await _check_project_selection_required(project):
        return "Please select a project first using select_project_for_session('project_name')."

    async with await _get_db_for_project(project) as conn:
        session_id = await _get_session_id_for_project(conn, project)

        # Generate preview (first 200 chars)
        preview = content[:200].replace('\n', ' ') + "..." if len(content) > 200 else content

        # Extract key concepts (simple keyword extraction for now)
        # In a real system, this would use an LLM or NLP lib
        key_concepts = ", ".join(list(set([w for w in topic.split('_') if len(w) > 3])))
        ...
        # Check if topic exists to determine version
        rows = await conn.execute("""
            SELECT version FROM context_locks
            WHERE label = $1 AND session_id = $2
        ...
        if rows:
            ...
            try:
                v_num = int(last_version.replace('v', ''))
                new_version = f"v{v_num + 1}"
            except:
                new_version = f"{last_version}.1"
        else:
            new_version = "v1"
        ...
        await conn.execute_update("""
            INSERT INTO context_locks (
                ...
                version, locked_at, metadata, last_accessed, access_count
            ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, 1)
        """, [
            session_id, topic, content, preview, key_concepts,
            new_version, time.time(), json.dumps(metadata), time.time()
        ])

        # Generate embedding (async if possible, or background task)
        # For now, we skip embedding generation in async path or need to make it async
        # TODO: Trigger async embedding generation

        return f"Context '{topic}' locked (version: {new_version})"


# Helper functions for recall_context (extracted for DRY and testability)

async def _fetch_context_by_version(conn, topic, version, session_id):
    """
    Query context_locks table for a specific topic and version (ASYNC).
    ...
    if version == "latest":
        ...
        rows = await conn.execute("""
            SELECT id, content, preview, key_concepts, version, locked_at, metadata
            FROM context_locks
            WHERE label = $1 AND version = $2 AND session_id = $3
...
async def recall_context(
    ...
    **Performance note:**
    ...
    async with await _get_db_for_project(project) as conn:
        ...
        row = await _fetch_context_by_version(conn, topic, version, session_id)
...
@mcp.tool()
async def update_context(topic: str, updates: str, project: str = None) -> str:
    """
    Append updates to an existing context.
    Creates a new version with the appended content.

    Args:
        topic: The label of the context to update
        updates: The content to append
        project: Project name (optional)
    """
    if await _check_project_selection_required(project):
        return "Please select a project first using select_project_for_session('project_name')."
    ...
    async with await _get_db_for_project(project) as conn:
        ...
        # Get latest version
        rows = await conn.execute("""
            SELECT content, version, metadata
            FROM context_locks
            WHERE label = $1 AND session_id = $2
            ORDER BY locked_at DESC LIMIT 1
        """, [topic, session_id])
        ...
        if not rows:
            return f"Context '{topic}' not found. Use lock_context to create it."
        ...
        # Update preview
        preview = new_content[:200].replace('\n', ' ') + "..." if len(new_content) > 200 else new_content
        ...
        """, [
            session_id, topic, new_content, preview, "",  # Keep key concepts empty or re-extract
            new_version, time.time(), json.dumps(metadata), time.time()
        ])

        return f"Context '{topic}' updated (version: {new_version})"

@mcp.tool()
async def unlock_context(topic: str, project: str = None) -> str:
    """
    Unlock (delete) a context and all its versions.

    Args:
        topic: The label of the context to unlock
        project: Project name (optional)
    """
    if await _check_project_selection_required(project):
        return "Please select a project first using select_project_for_session('project_name')."
    ...
    async with await _get_db_for_project(project) as conn:
        ...
        return f"Context '{topic}' unlocked (all versions deleted)"

# Add these functions after unlock_context() in claude_mcp_hybrid.py (around line 1831)

@mcp.tool()
async def wake_up(project: str = None) -> str:
    """
    Start or resume a session. Loads previous handover if available.
    ...
    # Auto-load handover (already handled by _check_project_selection_required, but explicit call is fine)
    handover = await _auto_load_handover()
    
    if handover:
        return f"🌅 Waking up in project '{project or 'default'}'...\n\n{handover}"
    
    return f"🌅 Waking up in project '{project or 'default'}'. Ready to work."

@mcp.tool()
async def check_contexts(project: str = None) -> str:
    """
    Check for critical (always_check) contexts that apply to the current session.
    Run this periodically or when starting a new task.
    ...
    async with await _get_db_for_project(project) as conn:
        ...
        rows = await conn.execute("""
            SELECT label, preview, version, metadata
            FROM context_locks
            WHERE session_id = $1
        ...
        if not rows:
            return "No critical contexts found."

        results = []
        for row in rows:
            results.append(f"- {row['label']} (v{row['version']}): {row['preview']}")

        return "Critical Contexts:\n" + "\n".join(results)
...
async def _try_semantic_search(conn, query: str, priority: Optional[str], tags: Optional[str], limit: int) -> Optional[dict]:
    """
    Attempt semantic search using embeddings (ASYNC).
    
    DISABLED temporarily during async migration until SemanticSearch service is updated.
    """
    # TODO: Implement AsyncSemanticSearch
    return None


def _build_keyword_search_query(query: str, priority: Optional[str], tags: Optional[str], limit: int) -> tuple:
    """
    Build SQL query and parameters for keyword search (ASYNC compatible).
    ...
    # Add priority filter
    if priority:
        sql += f" AND (metadata::json)->>'priority' = ${param_idx}"
    ...
    if tags:
        ...
        for tag in tags.split(','):
            tag = tag.strip()
            tag_conditions.append(f"(metadata::json)->>'tags' LIKE ${param_idx}")
            params.append(f'%{tag}%')
            param_idx += 1
...
async def search_contexts(
    ...
    **Use cases:**
    ...
    async with await _get_db_for_project(project) as conn:
        ...
        if use_semantic:
            semantic_result = await _try_semantic_search(conn, query, priority, tags, limit)
            if semantic_result:
...
async def upload_to_storage(
    ...
    try:
        ...
        # Upload
        success = storage.upload_file(str(path_obj), object_name)
        
        if success:
            final_name = object_name if object_name else path_obj.name
            url = storage.get_file_url(final_name)
            return json.dumps({
                "status": "success",
                "file": final_name,
                "url": url,
...
async def ingest_document(
    ...
    try:
        ...
        try:
            text_content, file_type = processor.extract_text_from_file(str(path_obj))
        except Exception as e:
            return json.dumps({
        ...
        embedding_text, was_truncated = processor.chunk_text_if_needed(text_content)
        
        # Step 3: Upload file to S3
        logger.info(f"Uploading {path_obj.name} to S3...")
        try:
            object_name = f"{project_name}/{path_obj.name}"
            storage.upload_file(str(path_obj), object_name)
            s3_url = storage.get_file_url(object_name)
        except Exception as e:
            return json.dumps({
        ...
        try:
            embedding_vector = voyage.generate_embedding(embedding_text, model="voyage-3-lite")
        except Exception as e:
        ...
        # Prepare metadata
        file_metadata = processor.get_file_metadata(str(path_obj))
...
if __name__ == "__main__":
    ...
    try:
        ...
        session_id = asyncio.run(_init_local_session())
